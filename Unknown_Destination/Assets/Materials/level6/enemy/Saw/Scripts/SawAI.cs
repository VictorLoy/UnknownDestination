...
public class SawAI : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		float distance = Vector2.Distance (transform.position, GameObject.Find ("Hero").transform.position);
		//monitor direction of the player
		Vector2 direction = transform.position - GameObject.Find("Hero").transform.position;
